...
//             lowest allowable carrier freq             19k
//             highest allowable carrier freq            88k
//             frequency resolution                     1.0k
//             carrrier range as % of Fs              8.20%  35.16%
#define        CARRIER_LOW                               21        // ((CARRIER_FREQ_LOW * FFT_LEN) + SAMPLE_FREQ-1) / SAMPLE_FREQ
#define        CARRIER_HIGH                              90        // (CARRIER_FREQ_HIGH * FFT_LEN) / SAMPLE_FREQ
#define        CARRIER_LEN                               70        // CARRIER_HIGH - CARRIER_LOW + 1
...
//             num bits / frame                         140        bits
//             AFE_SIZE                                   3        words
//             RX_SRC_INC                                 2        words
#define        NUM_DATA_BLOCKS                           16        // CEILING(num_encoded_bits/num_bits_per_frame)
#define        NUM_SYMBOLS                             1120        // total number of symbols in message
...
#define        DATA_BUFFER_LEN                           70        // words = NUM_SYMBOLS*BITS_PER_SYMBOL*encoder_rate/WORD_LEN

//             preamble+sync length                    2560        NUM_PRE_FRAMES*FFT_LEN + SYNC_FIELD_LEN
//             data field length                      15472        NUM_DATA_FRAMES*(CYCLIC_PREFIX_LEN + FFT_LEN) + WINDOW_LEN
//             receive waveform in Matlab:            19332
//             number of receive buffer frames            8
#if            COMPILE_MODE == MEX_COMPILE
       #define TX_BUFFER_LEN                          18032L       // samples = preamble + data
...
#define        PACKET_TIME                             1127        // number of 16 sample periods in packet
...
#define        DDPHASE_HI_LIM                            66        // upper and lower thresholds for preamble detect
#define        DDPHASE_LO_LIM                            28        //

//---- Frame Align parameters --------------------------------------------------------------
//             Align done % of CARRIER_LEN              55%        //
#define        PHASE_MEAS_LEN                             5        // number of subcarriers to use to compare phase
#define        ALIGN_DONE_CRITERIA                       39        // Criteria for desiding if we've run into the sync field
