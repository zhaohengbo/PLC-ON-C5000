...
//             lowest allowable carrier freq             29k
//             highest allowable carrier freq            88k
//             frequency resolution                     1.0k
//             carrrier range as % of Fs             12.11%  35.16%
#define        CARRIER_LOW                               31        // ((CARRIER_FREQ_LOW * FFT_LEN) + SAMPLE_FREQ-1) / SAMPLE_FREQ
#define        CARRIER_HIGH                              90        // (CARRIER_FREQ_HIGH * FFT_LEN) / SAMPLE_FREQ
#define        CARRIER_LEN                               60        // CARRIER_HIGH - CARRIER_LOW + 1
...
//             num bits / frame                         120        bits
//             AFE_SIZE                                   3        words
//             RX_SRC_INC                                 2        words
#define        NUM_DATA_BLOCKS                           18        // CEILING(num_encoded_bits/num_bits_per_frame)
...
//             data field length                      17404        NUM_DATA_FRAMES*(CYCLIC_PREFIX_LEN + FFT_LEN) + WINDOW_LEN
//             receive waveform in Matlab:            21264
//             number of receive buffer frames            8
#if            COMPILE_MODE == MEX_COMPILE
       #define TX_BUFFER_LEN                          19964L       // samples = preamble + data
...
#define        PACKET_TIME                             1248        // number of 16 sample periods in packet
...
//             double difference phase high thr         42%        // if > this % of bits in error reset packet det counter
//                                           lo         29%        // if < this % of bits in error inc packet det counter
...
#define        DDPHASE_HI_LIM                            50        // upper and lower thresholds for preamble detect
#define        DDPHASE_LO_LIM                            35        //

//---- Frame Align parameters --------------------------------------------------------------
//             Align done % of CARRIER_LEN              23%        // if > this % of bits in error, must be sync frame
#define        PHASE_MEAS_LEN                             5        // number of subcarriers to use to compare phase
#define        ALIGN_DONE_CRITERIA                       28        // Criteria for desiding if we've run into the sync field
